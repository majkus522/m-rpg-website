...
function draw()
{
    ...
    let max = Math.max(str, agl, chr, int);
    svg.querySelector("polygon").setAttribute("points", (width / 2) + "," + ((heigth / 2) - (heigth / 2 * str / max) + 5) + " " + (width - ((width / 2) - (width / 2 * agl / max)) - 5) + "," + (heigth / 2) + " " + (width / 2) + "," + (heigth - ((heigth / 2) - (heigth / 2 * chr / max)) - 5) + " " + ((width / 2) - (width / 2 * int / max) + 5) + "," + (heigth / 2));
